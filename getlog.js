...
function writeConfig(_obj) {
	fs.writeFile(configFile, JSON.stringify(_obj, null, 4), function(err) {
	    ...
	    console.log(`Saved Configfile: ${configFile}\n ${JSON.stringify(_obj, null, 4)} - File saved ...\n\n`);
	});
}
// let buff = new Buffer(data);
// let auth = "Basic " + buff.toString('base64');

// var auth = "Basic " + String(username + ":" + password).toString('base64');
		...
		res.on('data', (chunk) => {
	  	...
	  	res.on('end', () => {
	  		...
	  		if (obj.result.length == 1)
	  			...
	  			console.log("Could not log in, check config file ...\n\n");
	  			return false;
	  		}
	  	});
	});
